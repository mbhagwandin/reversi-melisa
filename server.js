       ...
       /* Check that the data coming fro the client is correct*/
        ...
        if ((typeof requested_user == 'undefined') || (requested_user === null) || (requested_user === "")){
         ...
         if ((typeof username == 'undefined') || (username === null) || (username === "")){
            response = {
                result: 'fail',
                message: 'the user that was invited does not have a nane registered'
            }
            socket.emit('invite_response',response);
            serverLog('invite command failed', JSON.stringify(response));
            return;
       ...
       /* Check that the data coming fro the client is correct*/
        ...
        if ((typeof requested_user == 'undefined') || (requested_user === null) || (requested_user === "")){
         ...
         if ((typeof username == 'undefined') || (username === null) || (username === "")){
            response = {
                result: 'fail',
                message: 'the user that was uninvited does not have a nane registered'
            }
            socket.emit('uninvited',response);
            serverLog('uninvite command failed', JSON.stringify(response));
            return;
         ...
         io.in(room).allSockets().then((sockets) => {
             ...
             /* Uninvitee is in the room */
                ...
                serverLog('unnvite command succeeded', JSON.stringify(response));
              }
         });
    });
       ...
       /* Check that the data coming fro the client is correct*/
        ...
        if ((typeof requested_user == 'undefined') || (requested_user === null) || (requested_user === "")){
         ...
         if ((typeof username == 'undefined') || (username === null) || (username === "")){
            response = {
                result: 'fail',
                message: 'the user that was engaged to play does not have a nane registered'
            }
            socket.emit('game_start_response',response);
            serverLog('game_start command failed', JSON.stringify(response));
            return;
...
function check_line_match(color, dr, dc, r, c, board){
    ...
    /* Check to make sure we aren't oging to walk off the board */
    if ((r + dr < 0) || (r + dr > 7)) {
        return false;
    }
    if ((c + dc < 0) || (c + dc > 7)) {
